  ...
  areas: [ 
    {
      ...
      value: 12419293,
      selectable: true,
      href: 'gallery.html?conf=drupal/juicebox/xml/viewsstyle/galleries/page_2/46.xml'
    }, {
      id: "US-IN",
      value: 6080485
    }, {
...
var dataProviderWorld = {
  map: "worldLow",
  areas: [
    {
      id: "US",
      value: 10000,
      linkToObject: dataProviderUsa,
    }
  ]
};
...
map.addListener("clickMapObject", function (event) {
  // Handle special click events on map ids.
  if ( event.mapObject.id == "backButton" ) {
    map.dataProvider = dataProviderWorld;
    map.validateData();
    map.validateNow();
    return;
  }
  if ( event.mapObject.id == "US" ) {
    map.dataProvider = dataProviderUsa;
    map.validateData();
    map.validateNow();
    return;
  }
  // Otherwise we direct the target into the appropriate parent (modal for
  // desktop and normal for mobile).
  var href = event.mapObject.href;
  // Don't do anytying if this is not a real link.
